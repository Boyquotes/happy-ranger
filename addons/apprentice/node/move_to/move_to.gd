...
## 在这个距离内则视为到达了位置
@export var arrive_distance := 100.0 : 
	set(v):
		arrive_distance = v
		_arrived_squared_distance = pow(arrive_distance, 2)
## 更新移动方向的间隔时间
@export var update_direction_time := 1.0:
	set(v):
		update_direction_time = v
		_update_timer.wait_time = update_direction_time


@onready var _arrived_squared_distance := pow(arrive_distance, 2)
@onready var _to_pos : Vector2 = global_position
...
func to(global_pos: Vector2) -> void:
	if global_pos != _to_pos or not is_physics_processing():
		_to_pos = global_pos
		_update_move_direction()
		_update_timer.start()
