...
class HasEnemy:
	...
	func _init(area_2d: Area2D) -> void:
		...
		var entered = func(node: Node):
			if node.is_in_group("enemy_test"):
				root.set_global_value("enemy", node)
		var exited = func(node: Node):
			if node.is_in_group("enemy_test"):
				root.set_global_value("enemy", null)
	...
	func do() -> bool:
		return root.get_global_value("enemy") != null
...
func move_to_player():
	var enemy = root.get_global_value("enemy") as Node2D
	if enemy:
...
func linger():
	var enemy := root.get_global_value("enemy") as Node2D
	var host_pos = root.get_global_value("host").global_position
	var target_pos : Vector2
	if enemy:
		target_pos = enemy.global_position
	else:
		target_pos = root.get_global_value("linger_target_pos", Vector2.INF)
		if (target_pos == Vector2.INF 
			or MathUtil.is_in_distance(target_pos, host_pos, 20)
		):
			target_pos = MathUtil.random_vector2(host_pos).random_position(50, 150)
	
	root.set_global_value("linger_target_pos", target_pos)
...
func _ready() -> void:
	...
	var self_path = FileUtil.get_global_path(self.get_script().resource_path)
	var document = Document.parse_by_path(
		self_path.path_join("doc/document.txt"), 
		self, config, func(document: String):
			return document.replace("\t", "    ").strip_edges()
	)
	
	var root = document.root as BTRoot
	root.set_global_value("host", player)
