...
static func _get_script_data_cache(script: Script) -> Dictionary:
	return DataUtil.singleton_dict("ScriptUtil__get_script_data_cache")
...
static func get_built_in_class (_class: StringName):
	if not ClassDB.class_exists(_class):
		return null
	var _class_db = DataUtil.singleton_dict("ScriptUtil_get_built_in_class")
...
static func get_script_class(_class: StringName):
	if ClassDB.class_exists(_class):
		return null
	var _class_db = DataUtil.singleton_dict("ScriptUtil_get_script_class")
	return DataUtil.get_value_or_set(_class_db, _class, func():
		var script = GDScript.new()
		script.source_code = "var type = " + _class
		if script.reload() == OK:
...
static func create_script(source_code: String) -> GDScript:
	var data = DataUtil.singleton_dict("ScriptUtil_create_script")
...
static func get_script_scene(script: GDScript) -> PackedScene:
	var data = DataUtil.singleton_dict("ScriptUtil_get_script_scene")
	if data.has(script):
		return data[script]
	else:
...
## 获取对象类名 
static func get_object_class_name(object: Object) -> StringName:
	if not object:
		return &""
	if object is Script:
		if object.has_source_code():
			var reg = RegEx.new()
			reg.compile("class_name\\s+(\\w+)")
			var result = reg.search(object.source_code)
			if result:
		...
		if object.get_script():
			return get_object_class_name(object.get_script())
		else:
			return object.get_class()
