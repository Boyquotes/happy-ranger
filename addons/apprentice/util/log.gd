...
static func prompt(params, sep:=DEFAULT_SEPARATOR):
	...
	if Engine.is_editor_hint():
		color = EditorUtil.get_editor_setting_property("interface/theme/accent_color")
	print_rich("[color=#%s]%s[/color]" % [color.to_html(false), _format_str(params, sep)])
...
static func debug(object: Object = null):
	if object:
		var script : Script
		if object is Script:
			script = object
		...
		if script:
			print_rich("[color=" + Color.DARK_ORANGE.to_html() + "][ DEBUG ] " + script.resource_path + "[/color]" )
		else:
			printerr("这个对象没有脚本")
			print_debug("")
	else:
		print_rich("[color=" + Color.DARK_ORANGE.to_html() + "][ DEBUG ] " + get_stack()[1]["source"] + "[/color]" )


static func print_list(list: Array, callable: Callable = Callable()):
	if callable.is_valid():
		for i in list:
			print(callable.call(i))
	else:
		for i in list:
			print(i)
...
static func ln(format_text: String, params = []):
	if not params is Array:
		params = [params]
	print_rich(format_text % params)
