...
static func _get_group_data(group) -> Dictionary:
	var data = DataUtil.singleton_dict("Group__get_data")
	return DataUtil.get_value_or_set(data, group, func(): {})

static func _get_node_groups(node: Node) -> Array:
	var data = DataUtil.singleton_dict("Group__node_groups")
...
## 节点是否在这个组中
static func has_node(group, node: Node) -> bool:
...
static func calls(group, method: StringName, arg_arry: Array = []):
