...
class RoomBlockMapData:
	...
	func _init(
		...
		for coords in exists_sub_room_coords:
			...
			for dir in dir_coords_list_map.keys():
				...
				for conn_data in conn_data_list:
					...
					var center : Vector2i = MathUtil.get_center(from, to)
		...
		# 记录这些方向都有的房间
		for directions in FuncUtil.combination(MathUtil.get_four_directionsi()):
...
class NineGridRoomDataGenerator:
	...
	func _init(start_pos: Vector2i):
		...
		FuncUtil.path_move(first_coords, MathUtil.get_four_directionsi()
		, func(coords: Vector2i):
			...
			if is_can_move_to.call(coords):
				...
				# 输出可移动方向
#				Log.print_format([
#					MathUtil.get_direction_as_name(coords), 
#					coords, 
#					MathUtil.get_nine_directions().find(Vector2(coords)) + 1,
#					around_directions.map(func(item): return str(MathUtil.get_direction_as_name(item)) )
#				], ["%-15s", "%-10s"])
...
class NineRoomGenerator:
	...
	static func generate_by_params(
		...
		return generate_by_data(
			...
			NineGridRoomDataGenerator.new(MathUtil.get_eight_directionsi().pick_random()),
			to_tilemap,
			to_room_offset,
		)
