...
static func execute_or_enter_tree(node: Node, callback: Callable):
...
static func call_once_in_tree(signal_or_node, callback: Callable, params: Array = []):
...
static func auto_inject(root: Node, by_name: bool = true, by_class: bool = false):
	...
	if not root.is_inside_tree():
		root.tree_entered.connect(call, Object.CONNECT_ONE_SHOT)
	else:
		call.call()
...
##[/codeblock]
static func forexec(list: Array, callback: Callable) -> void:
	for item in list:
		callback.call(item)
...
static func for_recti(rect: Rect2i, callback: Callable) -> void:
	for y in range(rect.position.y, rect.end.y + 1):
		for x in range(rect.position.x, rect.end.x + 1):
...
##[br][code]callback[/code]  用于合并的方法。这个方法需要有 4 个参数：
...
static func merge_dict(from: Dictionary, to: Dictionary, callback: Callable) -> void:
	...
	call[0] = func(to_previous: Dictionary, to_value, from_key, from_value):
		call[0].call(to_previous, to_value, from_key, from_value)
	...
	for from_key in from:
		call[0].call( 
			to, 
			to.get(from_key) if to is Dictionary else null, 
			from_key, from[from_key]
		)
...
##[br]
##[br][b]注意：[/b]如果类型为 [Dictionary] 时，参数数据格式为
##[codeblock]
##{"key": 键, "value": 值}
##[/codeblock]
...
##    if item_data.type == ItemType.CONSUMABLE:   # 消耗类物品放在第1位
##        return 1
##    elif item_data.type == ItemType.WEAPON:     # 武器类放在第2位
##        return 2
##    elif item_data.type == ItemType.DECORATIVE: # 饰品类放在第3位
##        return 3
##    else:
##         return INF
##)
##[/codeblock]
##[br]一般情况下物品类型都是枚举值，只用直接根据枚举值设置顺序：
##[codeblock]
...
static func set_of(list: Array) -> Array:
	var dict : Dictionary = {}
	for item in list:
		dict[item] = null
	if list.is_typed():
...
##[/codeblock]
static func sync_property(callback: Callable) -> void:
	var from := callback.get_object() as Node
	if from and not from.is_inside_tree():
		await from.ready 
	callback.call()
...
##[br][code]callback[/code]  回调方法，这个方法需要有一个 [Vector2i] 类型的参数接收回调
##[br][code]zoom_range[/code]  缩放范围，从外边还是内边进行遍历
static func for_rect_around(rect: Rect2, callback: Callable, zoom_range: int = 0):
	if zoom_range != 0:
		rect = rect.grow(zoom_range)
