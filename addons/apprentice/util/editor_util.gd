...
static func get_editor_interface() -> EditorInterface:
	return DataUtil.singleton("EditorUtil_get_editor_interface", func(): 
		# 默认值
		return ScriptUtil.get_instance("EditorScript").get_editor_interface() 
	)

##  获取插件节点
##[br][code]return[/code]  
static func get_editor_plugin() -> EditorPlugin:
	return DataUtil.singleton("EditorUtil_get_editor_plugin", func():
		return ScriptUtil.get_instance("EditorPlugin")
	)
