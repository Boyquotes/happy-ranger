...
#============================================================
## 用于保存数据文件
class_name CollectionPanelDataFile
extends Object
...
static func instance(_file_path: String, default_data: Dictionary = {}) -> CollectionPanelDataFile:
	...
	if not data.has(_file_path):
		var data_file = CollectionPanelDataFile.new()
		data_file._file_path = _file_path
		if FileAccess.file_exists(_file_path):
			var v = _read_as_bytes_to_var(_file_path)
			data_file._data = v
		else:
...
func get_value(key, default = null):
	if not _data.has(key) or _data[key] == null:
		_data[key] = default
	return _data[key]
...
## 替换数据
func replace(key, data: Dictionary):
	_data[key] = data
