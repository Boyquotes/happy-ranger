...
uniform float color_outline_scale = 2.0;
uniform float depth_outline_scale = 2.0;
uniform float depth_threshold : hint_range(0,10) = 2.5;
uniform float depth_multiplier = 1000.0;
uniform float edge_threshold : hint_range(0,1) = 0.04;
uniform float max_edge_alpha : hint_range(0,1) = 0.8;
uniform float edge_alpha_multiplier = 3.0;

uniform vec4 edge_color : hint_color = vec4(0,0,0,1);
uniform sampler2D bgTex : hint_albedo;

uniform bool depth_use_laplace = true;
uniform bool color_use_laplace = false;
uniform bool use_bg_texture = false;
